...
class PersonListTest:
  import Person.*
  val sequence: Sequence[Person] = Cons(Student("Mario", 2000), Cons(Teacher("Mirko", "PPS"), Cons(Teacher("Aguzzi", "PPS"), Nil())))
  val intSequence: Sequence[Int] = Cons(3, Cons(7, Cons(1, Cons(5, Nil()))))

  @Test def teacherCoursesShouldReturnCourses(): Unit =
    assertEquals(Cons("PPS", Cons("PPS", Nil())), teachersCourses(sequence))
